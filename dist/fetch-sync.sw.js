...
/***/ function(module, exports, __webpack_require__) {
	...
	function registerSync(sync) {
	  ...
	  var tag = sync.type === _constants.SyncTypes.PERIODIC_SYNC ? _extends({}, sync.options, { tag: sync.id }) : sync.id;
	
	  return self.registration[sync.type].register(tag);
	}
	...
	self.addEventListener(_constants.SyncTypes.SYNC, syncEvent);
	self.addEventListener(_constants.SyncTypes.PERIODIC_SYNC, syncEvent);
